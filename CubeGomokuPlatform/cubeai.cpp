...
CubeAI::CubeAI(int depth, int numOfGoodNodes, char chessBoard[15][15]){
    evaluator = Evaluate();
    this->depth = depth;
...
void CubeAI::search_several_good_nodes(char player,
                                       char board[MAX_LINE][MAX_LINE],
                                       Node *goodNodeArray, int numGoodNodes) {
...
// alpha-beta减枝搜索算法
int CubeAI::alphaBetaSearch(int search_depth, int alpha, int beta,
                            char player) {
  int score;  //估值结果
  ...
  search_several_good_nodes((player == MYSELF) ? RIVAL : MYSELF, chessBoard,
                            badNodes, 1);
  //如果赢了，不可能再在此基础上下下去
  ...
  for (int i = 0; i < numOfGoodNodes; ++i) {
    ...
    score = -alphaBetaSearch(search_depth + 1, -beta, -alpha,
                             (player == MYSELF) ? RIVAL : MYSELF);
