...
struct Node {
  ...
  bool operator<(const Node aNode) const { return this->value > aNode.value; }
};

class CubeAI {
public:
    CubeAI(int depth, int numOfGoodNodes, char chessBoard[15][15]);
    void search_several_good_nodes(
        char player, char board[MAX_LINE][MAX_LINE], Node *goodNodeArray,
        int numGoodNodes);  //贪心法先找出几个比较好的点
