    ...
    while (search_left_up || search_right_down) {
      if (search_left_up) {
        if (--tmp_x1 < 0 || --tmp_y1 < 0 ||
            chessBoard[tmp_x1][tmp_y1] != player) {
        ...
        if (++tmp_x2 >= MAX_LINE || ++tmp_y2 >= MAX_LINE ||
            chessBoard[tmp_x2][tmp_y2] != player) {
