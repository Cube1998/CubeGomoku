...
void PeopVSComp::config() {
  Configuration *aConfiguration =
      new Configuration(audio, p_move_first, difficulty_degree, this);
  aConfiguration->exec();
}
...
void PeopVSComp::exitGame() {
  ...
  reply = QMessageBox::question(
      this, "Warning", QString::fromUtf8("真的想退出吗?"),
  ...
  if (game_start) {
    for (int i = 0; i <= nodeStack.top; ++i) {
      if (nodeStack.nodes[i].color == "BLACK")
        painter.drawImage(nodeStack.nodes[i].x, nodeStack.nodes[i].y,
                          *blackImage);
      else
        painter.drawImage(nodeStack.nodes[i].x, nodeStack.nodes[i].y,
                          *whiteImage);
